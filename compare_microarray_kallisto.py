...
num_kallisto_files = int(sys.argv[3])
files = []
for i in range(4, 4+num_kallisto_files):
...
for file in files:
        ...
        for line in f:
            if line[0] != "t":
                ...
                if gene in kallisto_values:
                    ...
                    kallisto_values.update({gene:exp_list})
    ...
    for gene in seqc_values:
        if gene in kallisto_values:
            exps = np.array(kallisto_values[gene])
            kallisto.append(np.mean(exps, axis = 0) ) # Chisana uses + 0.5
    ...
    if (stats.pearsonr(seqc, kallisto)[1] <= 0.003):
        pearson.append(stats.pearsonr(seqc, kallisto)[0] )
    else:
        pearson.append(0)
    if (stats.spearmanr(seqc, kallisto)[1] <= 0.003):
        spearman.append(stats.spearmanr(seqc, kallisto)[0] )
    else:
        spearman.append(0)

    print (miss, len(seqc), len(kallisto))
    miss = 0
    iterator += 1
    it += 1
    if (it == 4) & (iterator < num_kallisto_files):
        Letter = Letters[0]
        Letters = Letters[1:]
        it = 0
